...
void main(tensor a, tensor b, int s, tensor output) {
 ...
 unsigned vec_len = 64;
 int64 sv = s; // scalar to vector broadcast
 ...
 for(int i = index_space_start[0]; i < index_space_end[0]; i++) {
    ...
    int128 c1 = v_i32_mul_b(b1, sv);
    int64 d1 = v_i32_add_b(a1, c1.v1); // ignoring overflow by considering .v1
    v_i32_st_tnsr(outputCoord, output, d1);
  }
}
