...
uchar256 U8Div(uchar256 a, uchar256 b) {
  ...
  float64 reciprocal_fb1 = reciprocal_cephes_fast_f32(fb.v1);
  float64 reciprocal_fb2 = reciprocal_cephes_fast_f32(fb.v2);
  float64 reciprocal_fb3 = reciprocal_cephes_fast_f32(fb.v3);
  float64 reciprocal_fb4 = reciprocal_cephes_fast_f32(fb.v4);
...
uchar256 ElseBlock(uchar256 base, uchar256 active) {
  ...
  uchar256 x = v_u8_sub_b(two_five_five, base);  // 255 - base
  uchar256 y = U8Div(x, active);  // (255 - base) / active
  uchar256 z = v_u8_sub_b(two_five_five, y);  // 255 - ((255 - base) / active)
  return z;
}
...
void main(tensor base, tensor active, tensor out) {
 ...
 for(int i = index_space_start[0]; i < index_space_end[0]; i++) {
    ...
    for (int j = index_space_start[1]; j < index_space_end[1]; j++) {
      ...
      uchar256 result = v_u8_sel_eq_u8_b(a, zero, two_five_five, else_expr);
