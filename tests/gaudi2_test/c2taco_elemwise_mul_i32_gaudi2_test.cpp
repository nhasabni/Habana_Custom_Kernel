...
void C2TacoElemwiseMulI32Gaudi2Test::c2taco_muleq_i32_reference_implementation(
...
int C2TacoElemwiseMulI32Gaudi2Test::runTest(uint32_t m)
{
    ...
    c2taco_muleq_i32_reference_implementation(a, b, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_ELEMWISE_MUL_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco MulEq I32 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco MulEq I32 test pass!!" << std::endl;
    return 0;
}
