...
void C2TacoElemwisePlusScalarMulI32Gaudi2Test::c2taco_elemwise_plus_scalar_mul_i32_reference_implementation(
        const int32_1DTensor& a, const int32_1DTensor& b, C2TacoElemwisePlusScalarMulI32Gaudi2::Param& param_def,
        int32_1DTensor& out)
{
   int coords[5] = {0};
   ...
   for (unsigned int i = 0; i < hidden_dim; i++) {
        coords[0] = i;
        out.SetElement(coords, a.ElementAt(coords) + (scalar * b.ElementAt(coords)));
   }
}

int C2TacoElemwisePlusScalarMulI32Gaudi2Test::runTest(uint32_t m, int scalar)
{
    ...
    c2taco_elemwise_plus_scalar_mul_i32_reference_implementation(a, b, param_def, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_ELEMWISE_PLUS_SCALAR_MUL_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco Elemwise Plus Scalar Mul I32 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco Elemwise Plus Scalar Mul I32 test pass!!" << std::endl;
    return 0;
}
