...
void C2TacoMatScalarMulI32Gaudi2Test::matscalar_mul_i32_reference_implementation(
        const int32_2DTensor& a, C2TacoMatScalarMulI32Gaudi2::Param& param, int32_2DTensor& out)
{
    int coords[5] = {0};
...
int C2TacoMatScalarMulI32Gaudi2Test::runTest(uint32_t m, uint32_t n, int scalar)
{
    ...
    C2TacoMatScalarMulI32Gaudi2::Param param_def;
    param_def.scalar = scalar;

    // execute reference implementation of the kernel.
    matscalar_mul_i32_reference_implementation(a, param_def, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_MATSCALAR_MUL_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco Matrix Scalar Mul test failed (at: " << element
                      << ", value: " << out.Data()[element] << ", exp:" << out_ref.Data()[element] << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco Matrix Scalar Mul test pass!!" << std::endl;
    return 0;
}
