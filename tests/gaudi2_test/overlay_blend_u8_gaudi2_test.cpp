...
void OverlayBlendU8Gaudi2Test::overlayblend_u8_reference_implementation(
        ...
        uint8_2DTensor& out)
{
    ...
    for (int row = 0; row < maxRows; row++) {
        for (int col = 0; col < maxCols; col++) {
            ...
            uint8_t b = base.ElementAt(coords);
            uint8_t a = active.ElementAt(coords);
            uint32_t two_times_a = 2 * a;

            if (b >= 128) {
                uint8_t x = two_times_a + b - ((two_times_a * b) / 255);
                uint8_t y = x - 255;
                out.SetElement(coords, y);
            } else {
                uint8_t x = (two_times_a * b) / 255;
...
int OverlayBlendU8Gaudi2Test::runTest(uint32_t m, uint32_t n)
{
    ...
    // reduce the range of active to prevent overflow in Screen8x8
    uint8_2DTensor active(tensor_shape);
    active.InitRand(0, 127);

    uint8_2DTensor out(tensor_shape);
    uint8_2DTensor out_ref(tensor_shape);

    // execute reference implementation of the kernel.
    overlayblend_u8_reference_implementation(base, active, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_OVERLAY_BLEND_U8]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "Overlay Blend U8 test failed!! (index:" << element << ")" << std::endl;
            return -1;
        }
    }
    std::cout << "Overlay Blend U8 test pass!!" << std::endl;
    return 0;
}
