...
void C2TacoVecScalarSubI32Gaudi2Test::c2taco_vecscalar_sub_i32_reference_implementation(
...
int C2TacoVecScalarSubI32Gaudi2Test::runTest(uint32_t m, int scalar)
{
    ...
    c2taco_vecscalar_sub_i32_reference_implementation(a, out_ref, param_def);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_VECSCALAR_SUB_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco VecScalar Sub I32 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco VecScalar Sub I32 test pass!!" << std::endl;
    return 0;
}
