...
void C2TacoElemwiseSubSquareI32Gaudi2Test::c2taco_sub_square_i32_reference_implementation(
        ...
        int32_1DTensor& out)
{
   ...
   for (unsigned int i = 0; i < hidden_dim; i++) {
        coords[0] = i;
        int diff = a.ElementAt(coords) - b.ElementAt(coords);
        out.SetElement(coords, diff * diff);
   }
}

int C2TacoElemwiseSubSquareI32Gaudi2Test::runTest(uint32_t m)
{
    ...
    c2taco_sub_square_i32_reference_implementation(a, b, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_ELEMWISE_SUB_SQUARE_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco ElemwiseSub Square I32 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco ElemwiseSub Square I32 test pass!!" << std::endl;
    return 0;
}
