...
void C2TacoLenSqI32Gaudi2Test::c2taco_lensq_i32_reference_implementation(
        const int32_1DTensor& a, int32_1DTensor& out)
{
   ...
   for (unsigned i = 0; i < a.Size(0); i++) {
      input_coords[0] = i;
      sum += (a.ElementAt(input_coords) * a.ElementAt(input_coords));
   }
   out.SetElement(output_coords, sum);
}

int C2TacoLenSqI32Gaudi2Test::runTest(uint32_t m)
{
    ...
    c2taco_lensq_i32_reference_implementation(a, sum_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_LENSQ_I32]);
    ...
    {
        std::cout << "C2Taco LenSq test failed!!" << std::endl;
        return -1;
    }
    #endif
    
    std::cout << "C2Taco LenSq test pass!!" << std::endl;
    return 0;
}
