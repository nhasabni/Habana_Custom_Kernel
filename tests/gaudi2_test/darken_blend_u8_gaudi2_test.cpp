...
void DarkenBlendU8Gaudi2Test::darkenblend_u8_reference_implementation(
...
int DarkenBlendU8Gaudi2Test::runTest(uint32_t m, uint32_t n)
{
    ...
    darkenblend_u8_reference_implementation(base, active, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_DARKEN_BLEND_U8]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "Darken Blend U8 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "Darken Blend U8 test pass!!" << std::endl;
    return 0;
}
