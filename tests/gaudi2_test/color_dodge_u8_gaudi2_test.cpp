...
void ColorDodgeU8Gaudi2Test::colordodge_u8_reference_implementation(
        ...
        uint8_2DTensor& out)
{
    ...
    for (int row = 0; row < maxRows; row++) {
        for (int col = 0; col < maxCols; col++) {
            ...
            if (active.ElementAt(coords) == 255) {
                out.SetElement(coords, (uint8_t) 255);
            } else {
                uint8_t x = base.ElementAt(coords) / ((uint8_t) 255 - active.ElementAt(coords));
...
int ColorDodgeU8Gaudi2Test::runTest(uint32_t m, uint32_t n)
{
    ...
    colordodge_u8_reference_implementation(base, active, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_COLOR_DODGE_U8]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "Color Dodge U8 test failed!! (index:" << element << ")" << std::endl;
            return -1;
        }
    }
    std::cout << "Color Dodge U8 test pass!!" << std::endl;
    return 0;
}
