...
void LinearBurnU8Gaudi2Test::linearburn_u8_reference_implementation(
        ...
        uint8_2DTensor& out)
{
    ...
    for (int row = 0; row < maxRows; row++) {
        for (int col = 0; col < maxCols; col++) {
            ...
            out.SetElement(coords, (base.ElementAt(coords) + active.ElementAt(coords)) - 255);
        }
    }
}

int LinearBurnU8Gaudi2Test::runTest(uint32_t m, uint32_t n)
{
    ...
    linearburn_u8_reference_implementation(base, active, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_LINEAR_BURN_U8]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "Linear Burn U8 test failed!!" << std::endl;
            return -1;
        }
    }
    std::cout << "Linear Burn U8 test pass!!" << std::endl;
    return 0;
}
