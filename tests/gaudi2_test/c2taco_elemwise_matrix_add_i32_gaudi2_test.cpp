...
void C2TacoElemwiseMatrixAddI32Gaudi2Test::elemwise_matrix_add_i32_reference_implementation(
...
int C2TacoElemwiseMatrixAddI32Gaudi2Test::runTest(uint32_t m, uint32_t n)
{
    ...
    elemwise_matrix_add_i32_reference_implementation(a, b, out_ref);
    ...
    strcpy(m_in_defs.nodeName, kernelNames[GAUDI2_KERNEL_C2TACO_ELEMWISE_MATRIX_ADD_I32]);
    ...
    for (int element = 0 ; element <  out_ref.ElementCount() ; element++)
    {
        ...
        {
            std::cout << "C2Taco Elemwise Matrix Add test failed (at: " << element
                      << ", value: " << out.Data()[element] << ", exp:" << out_ref.Data()[element] << std::endl;
            return -1;
        }
    }
    std::cout << "C2Taco Elemwise Matrix Add test pass!!" << std::endl;
    return 0;
}
